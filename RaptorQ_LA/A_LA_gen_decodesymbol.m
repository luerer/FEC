function DecSym = A_LA_gen_decodesymbol(layer_t,RecInterSym)
...
for ii = 1:layer_t
	...
	elseif ii>1 
		ESI = Ite;
		for jj = 1:ii-1
			ESI = ESI+prime_N(index(jj),1);
		end
    end
%     disp (index)
    G_temp = gen_G(prime_N(layer_t+1-ii,1),ESI);
...
%     end
	...
	decode_temp = [G_temp decode_temp];
end

% [row1 col1] = size(encode_temp)
% [row2 col2] = size(InterSym(:).'')
DecSym = GF_multiply(decode_temp,RecInterSym(:).'');
